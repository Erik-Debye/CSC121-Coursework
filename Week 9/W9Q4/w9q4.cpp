...
int main()
{
    ...
    int source{}, dest{};
    char value{};
    //flag for whether value removed from source
    bool sourceFlag{};
    //counter
    int moves{ 0 };
    //solution
    vector<char> solution{' ', 'A', 'B', 'C'};

    //Vectors (one per pole)                                                Might need a " " at the begining for for each loops
    vector<char> T1{' ', 'A', 'B', 'C'}, T2{' '}, T3{' '};
    ...
    do {
        ...
        switch (dest) {
        case towerOne:
            //value must be larger than existing
    ...
    if (moves > 7) {
        cout << "\n\tLeast moves required to solve this puzzle is 7.\n";
